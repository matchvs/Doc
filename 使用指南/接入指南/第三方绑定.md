...
- 调用 code2accessToken 接口获取openID ，获取openID 操作最好 是在自己的服务端接口获取，然后再返回。 code2accessToken 这个接口需要你的微信小游戏 secret。获取openID 示例：

```javascript
wx.request({
    url: "这个是您的获取openID 地址接口",
    method: "GET",
    data: {
...
获取openID可参考 [微信小游戏开发文档](https://developers.weixin.qq.com/minigame/dev/document/open-api/login/wx.login.html) code2accessToken 接口和 wx.login 接口。



## 调用绑定接口

获取到的微信用户信息和 openID 再调用 matchvs 第三方账号绑定接口 获取用户信息。这里以 egret 代码请求为例：
...
> 注意：调用第三方绑定接口请求参数userID传0，如果是第一次绑定，接口会返回新的 userID ，如果已经绑定过，接口会返回您绑定了的 userID信息。
...
在调用 thirdBind.do  接口时，传入的 openID 需要有效性检测。官网-> 控制台->我的游戏->设置-> 安全校验 配置你自己进行有效性检测的接口地址。配置好回调地址后，调用 thirdBind.do  就会回调这个地址检测 openID信息是否有效。如果没有配置 安全校验是不校验的。
