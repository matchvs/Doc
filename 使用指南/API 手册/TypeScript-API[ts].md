...
2、在工程文件 `egretProperties.json` 添加库路径，在工程目录下使用命令行 egret build -e 编译工程，可以看到在 libs--> modules下看到 matchvs这个库。或者 配置好 `egretProperties.json` 文件后在 Wing 工具中 选择清理，再编译就能 在 libs-->modules 看到我们的库文件`matchvs` 。
