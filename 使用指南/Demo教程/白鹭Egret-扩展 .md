...
| 参数   | 含义                                                         |
| ------ | ------------------------------------------------------------ |
| filter | maxPlayer最大人数<br>mode：模式<br>canWatch:是否可观战<br>roomProperty:房间属性 |

查看房间列表-扩展接口回调

```javascript
response.getRoomListExResponse = function (rsp){
    console.log("[Rsp]getRoomListExResponse->status:"+rsp.status+" total:"+rsp.total);
        for (var i = 0 ; i < rsp.roomAttrs.length; i++){
            console.log("[Rsp]getRoomListExResponse->roomAttrs:Index"+i+" roomID:"+rsp.roomAttrs[i].roomID);
        }
}
```
...
```javascript
response.setRoomPropertyResponse = function(rsp){
        console.log("status:"+rsp.status+" roomID:"+rsp.roomID + " userID:"+rsp.userID + " roomProperty:"+ rsp.roomProperty);
    };
```
